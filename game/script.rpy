...
label intro:
    ...
    call chooseRoute from _call_chooseRoute
...
label chooseRoute:
    menu:
        ...
        "Crafts Room":
            call craftsRoom from _call_craftsRoom
        "Game Room":
            call gameRoom from _call_gameRoom
        "Communal Lounge" if seenAnnabelle == False:
            call communalLounge from _call_communalLounge
...
label craftsRoom:
    ...
    if(mi_score > 0):
        call miriam_good from _call_miriam_good
    elif (mi_score < 0):
        call miriam_bad from _call_miriam_bad
    #Call a random ending if the score is 0
    else:
        $ chooseEnding = renpy.random.randint(1,2)
        if(chooseEnding == 1):
            call miriam_good from _call_miriam_good_1
        else:
            call miriam_bad from _call_miriam_bad_1
...
label gameRoom:
    ...
    #Determine the ending
    if(hl_score > 0):
        call helen_good from _call_helen_good
    elif (hl_score < 0):
        call helen_bad from _call_helen_bad
    #Call a random ending if the score is 0
    else:
        $ chooseEnding = renpy.random.randint(1,2)
        if(chooseEnding == 1):
            call helen_good from _call_helen_good_1
        else:
            call helen_bad from _call_helen_bad_1
    return
...
label communalLounge:
    ...
    call chooseRoute from _call_chooseRoute_1

    #Determine the ending
    if(an_score > 0):
        call annabelle_good from _call_annabelle_good
    elif (an_score < 0):
        call annabelle_bad from _call_annabelle_bad
    #Call a random ending if the score is 0
    else:
        $ chooseEnding = renpy.random.randint(1,2)
        if(chooseEnding == 1):
            call annabelle_good from _call_annabelle_good_1
        else:
            call annabelle_bad from _call_annabelle_bad_1
    return
